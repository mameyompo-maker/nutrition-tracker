...
const PROVIDER_GUIDES = {
  gemini: {
    ...
    // 最初に読む説明。手順に入る前の不安を解いておく。
    // このアプリは利用者自身のAIにつなぐ方式なので、ここでつまずくと
    // 何も使えないまま終わってしまう。手順書の丁寧さが、そのまま使えるかどうかを決める。
    intro: {
      title: "はじめに ─ これから何をするのか",
      body: [
        "**APIキーは「あなた専用の合鍵」です。** これをアプリに預けると、アプリがあなたの代わりにGoogleのAIへ写真を見せて、栄養を答えてもらえるようになります。",
        "**費用はかかりません。** これから取るのは無料の鍵です。カード番号を聞かれる場面はありません(もし聞かれたら、それは押す必要のないボタンです。手順の中で目印を書いておきます)。",
        "**難しい知識は要りません。** 押すボタンは全部で3つほどです。ただし**画面が英語で出ることがあります。**そのときのために、押すボタンの英語表記も併記しました。",
        "**途中でやめても大丈夫です。** 何かを壊すことはありません。分からなくなったら、この画面を閉じて最初からやり直せます。",
      ],
    },

    steps: [
      { text: "下のボタンから Google AI Studio のAPIキー画面を開きます。**別のタブで開くので、この手順書は残ります。**",
        link: { url: "https://aistudio.google.com/apikey", label: "APIキー画面を開く" } },
      { text: "Googleアカウントでログインします。**普段お使いのアカウントで構いません。**新しく作る必要はありません。" },
      { text: "初回は利用規約の同意画面が出ます。内容を確認して同意してください。**ここではまだ何も課金されません。**" },
      { text: "青い「**APIキーを作成**」ボタンを押します。英語表示なら「**Create API key**」です。画面の右上か中央にあります。" },
      { text: "プロジェクトを選ぶ画面が出たら、「**新しいプロジェクトでAPIキーを作成**」(英語なら **Create API key in new project**)を選びます。プロジェクトの名前は何でも構いません。" },
      { text: "**AIza** で始まる長い文字列が出ます。これが鍵です。となりのコピーボタン(四角が2枚重なった印)を押してコピーしてください。**手で打ち写す必要はありません。**" },
      { text: "このアプリに戻り、「APIキー」欄を長押しして「ペースト」を選びます。最後に「**接続テスト**」を押してください。「接続できました」と出れば完了です。" },
    ],

    // つまずいたときの逃げ道。ここが無いと、詰まった人はそのまま離れてしまう。
    faq: [
      { q: "画面が全部英語で、どこを押せばいいか分かりません",
        a: "探すのは「**Create API key**」という青いボタン1つだけです。それ以外は読まなくて構いません。見つからないときは、ブラウザの翻訳機能(Chromeなら右上の翻訳アイコン)で日本語にできます。" },
      { q: "「Set up Billing」「課金を有効にする」と出ました",
        a: "**押さないでください。**押さなくても無料で使えます。この案内は閉じて構いません。誤って進んでも、カード情報を入力しない限り課金は始まりません。" },
      { q: "コピーした鍵をどこかに失くしました",
        a: "同じ画面でもう一度作り直せます。**古い鍵は削除して構いません。**鍵は何本でも作れます。" },
      { q: "「接続できませんでした」と出ます",
        a: "多いのは3つです。**(1)** 貼り付けのときに前後に空白が混じった → 貼り直してください。**(2)** 鍵の全部がコピーできていない → コピーボタンを使ってください。**(3)** 作った直後で反映待ち → 1分ほど待って、もう一度お試しください。" },
      { q: "そもそもAPIキーを使いたくありません",
        a: "使わなくても記録できます。記録画面の「**手入力**」から、料理名と分かる数値だけで登録できます。また「APIキーを使わない方法」を使うと、お使いのAIアプリ(ChatGPTやGeminiのアプリ)に指示文を貼って、返ってきた答えをこのアプリに戻す形でも解析できます。" },
    ],
